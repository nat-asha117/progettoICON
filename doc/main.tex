...
L'utente può decidere di interagire con la knowledge base effettuando interrogazioni su quest’ultima.
%

\noindent
Per poterlo fare, bisogna inserire i dati relativi alla sua età, altezza e peso obbligatoriamente (rispettando i valori ed i controlli segnalati). Per i restanti valori, quali gtp, triglyceride,
HDL, hemoglobin, serum creatinine, dental caries, tartar, l'utente, se non si è a conoscenza di uno di essi (o di tutti) può scegliere di non inserirli e quindi effettuare una interrogazione "parziale".
...
\noindent
Se la probabilità di essere un fumatore è superiore al 50\%, il sistema premetterà all'utente di indicare dei suggerimenti, attraverso una predizione, di valori ai quali deve ambire per poter risultare un non fumatore.
